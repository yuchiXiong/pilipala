...
window.notice = function (title, messages, options = {type: 'primary'}) {
    const error_alert_dom = $("#error_alert");
    ...
    // * 添加DOM到页面
    $(`<div id="error_alert" class="alert alert-${options.type} alert-dismissible fade show fixed-top" role="alert">
        <strong>${title}</strong>
    ...
    // * 添加事件进入和退出动效
    $("#error_alert").animate({top: 0}, 450).delegate('.close', 'click', () => {
        $("#error_alert").animate({top: -200}, 450, null, function () {
            this.remove();
        })
    });
}